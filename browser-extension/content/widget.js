/* ShadowReply AI — Floating widget (Shadow DOM isolated) */
(function () {
  'use strict';
  ...
  if (location.hostname.includes('shadowreply.ai')) return;
  // Don't double-inject
  if (document.getElementById('shadowreply-widget-host')) return;

  // Check if user hid the widget
  ...
  function updateSelectionBar() {
    ...
    if (selectedText && selectedText.length > 3) {
      selText.textContent = selectedText.length > 100
  ...
  document.addEventListener('mouseup', () => {
    ...
    if (sel.length > 3) {
      selectedText = sel;
      // Send to background for popup access
      try {
